...
async function axiosJSON(endPoint, callback) {
  try {
    const response = await axios.get(endPoint);
    callback(response.data, arguments[2]);
  } catch {
    document.getElementById("root").innerHTML = error;
  }
}
...
function renderRepositories(jsonData, filteredItem) {
  ...
  axiosJSON(filteredJSONData.contributors_url, renderContributors);
}
...
window.onload = () => {
  ...
  axiosJSON(HYF_REPOS_URL, renderRepositories);

  // At start-up, it will be load select information
  axiosJSON(HYF_REPOS_URL, renderSelect);
  ...
  repositories.addEventListener("change", (event) => {
    ...
    axiosJSON(HYF_REPOS_URL, renderRepositories, selectedOption);
  });
};
